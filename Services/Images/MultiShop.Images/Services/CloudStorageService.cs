    ...
    {   
        ...
        public async Task DeleteFileAsync(string fileNameToDelete)
        {
            try
            {
                ...
                {
                    await storageClient.DeleteObjectAsync(_options.GCPStorageBucketName, fileNameToDelete);
                }
        ...
        public async Task<string> GetSignedUrlAsync(string fileNameToRead, int timeOutInMinutes = 30)
        {
            try
            {
                ...
                var signedUrl = await urlSigner.SignAsync(_options.GCPStorageBucketName, fileNameToRead, TimeSpan.FromMinutes(timeOutInMinutes));
        ...
        public async Task<string> UploadFileAsync(IFormFile fileToUpload, string fileNameToSave)
        {
            try
            {
                _logger.LogInformation($"Uploading: file {fileNameToSave} to storage {_options.GCPStorageBucketName}");
                using (var memoryStream = new MemoryStream())
                {
                    ...
                    {
                        // upload file stream
                        var uploadedFile = await storageClient.UploadObjectAsync(_options.GCPStorageBucketName, fileNameToSave, fileToUpload.ContentType, memoryStream);
                        _logger.LogInformation($"Uploaded: file {fileNameToSave} to storage {_options.GCPStorageBucketName}");
